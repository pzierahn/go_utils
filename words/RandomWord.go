...
func random(min, max int) int {
	// don't do that: rand.Seed(time.Now().Unix())
...
func randomWord(wordLength int) string {
	...
	return strings.ToLower(randomWord)
}

func addCharacter(startBiGram []string, desiredLength int, currentWord string, lookupBiGram []string, nextCharLookup [][][]string, flag int) string {
	mainIndex := getLookupIndex(currentWord, lookupBiGram)
	typee := 0

	if len(currentWord) == (desiredLength - 1) {
		typee = 1
	}

	for (mainIndex < 0) || (mainIndex > 263) || (len(nextCharLookup[mainIndex][typee]) <= 0) {

		if len(currentWord) == 2 {
			return startBiGram[indexGenerator(len(startBiGram))]
		}
	...
	return currentWord + getNextCharacter(typee, mainIndex, nextCharLookup)
}

func indexGenerator(arrayLength int) int {
	return random(0, arrayLength)
}

func getNextCharacter(typee int, mainIndex int, theCharacterVault [][][]string) string {
	inx := indexGenerator(len(theCharacterVault[mainIndex][typee]))
	return theCharacterVault[mainIndex][typee][inx]
}

func backtrack(theWord string, numberChars int) string {
	return string([]rune(theWord)[0:(len(theWord) - numberChars)])
}
...
func getLookupIndex(theWord string, lookupArray []string) int {
	index := len(theWord)
	...
	lookupCharacters := string([]rune(theWord)[(index - 2):])
	return indexOf(lookupCharacters, lookupArray)
}
